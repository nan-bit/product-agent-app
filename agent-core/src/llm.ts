 ...
 *
 * agent-core never imports Genkit, the Google SDK, or any provider directly —
 * it only asks an `LlmClient` to turn a prompt into a value matching a Zod
 * schema. Implement this interface to plug in any backend (a Genkit adapter is
 * shipped in `adapters/genkit.ts`, but a raw-SDK or mock implementation works
 * just as well). This is what keeps the "brain" portable.
 */
